...
export default function Approval({
  ...
  const permitApproveProps = usePermit({
    ...
    value: amount,
    spender: spender,
    tokenAddress: tokenConfig.addressOrName,
  ...
  const txButtonProps = useWrite({
    ...
    method:
      requireApproval && permitArgs
        ? getPermitMethod(chain.id, actionProps.method)
        : actionProps.method,
    ...
    if (amount.gt(0)) {
      if (requireApproval && amount.gt(allowance) && !permitArgs) {
        ...
        if (gasless) {
          setAction({
            ...permitApproveProps,
            label: permitApproveProps.loading ? "Approving..." : "Approve",
            loading: false,
